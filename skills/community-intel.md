# Skill: Community Intelligence

> **Purpose**: Gather and report on community sentiment, trends, and opportunities
> **Primary User**: Agent 09 (Resident Degen), Coordinator (Agent 00)
> **Use When**: Monitoring communities, writing intel reports, tracking sentiment

**Note**: This skill absorbed content from the archived Agent 10 (Infiltrator) persona.

---

## Intel Report Quick Format

```markdown
## Intel Report: [Date]

### Hot Topics
1. [Topic]: [Where] - [Our angle]

### Threats
- [Threat]: [Severity] - [Action needed]

### Opportunities
- [Opportunity]: [Window] - [Recommended action]

### Sentiment: [Positive/Neutral/Negative]
```

---

## Spaces to Monitor

| Priority | Space Type | Examples |
|----------|------------|----------|
| P0 | Crypto Twitter | Main CT, gacha CT, anime CT |
| P0 | Discord | Competitor servers, ecosystem servers |
| P1 | Telegram | Alpha groups, project chats |
| P1 | Reddit | r/gachagaming, relevant subs |
| P2 | YouTube/TikTok | Gacha creators, trends |

---

## Sentiment Analysis

### Positive Signals
- Organic praise/excitement
- Sharing our content
- Defending us in discussions
- Asking "when" questions (anticipation)

### Negative Signals
- Complaints patterns
- Comparisons to competitors (negative)
- "Dead game" sentiment
- FUD narratives spreading

### Neutral/Watch
- Questions seeking information
- Comparisons (objective)
- Wait-and-see sentiment

---

## Alert Triggers

| Trigger | Priority | Action |
|---------|----------|--------|
| Scam impersonating us | P0 | Immediate alert + human escalation |
| FUD going viral | P0 | Alert with context |
| Competitor major move | P1 | Document and report |
| Engagement opportunity | P1 | Flag to relevant agent |
| Trend we should join | P2 | Include in daily report |

---

## Engagement Rules

### DO
- Be a genuine community member
- Build relationships over time
- Share value before asking
- Stay in character consistently

### DON'T
- Shill obviously
- Get into fights
- Share confidential info
- Pretend to be multiple people

---

*"You can't understand a community from the outside. Get in there."*
